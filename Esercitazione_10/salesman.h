...
public:
    ...
    int select(){
        int selected = int(_itinera.size()*pow(_rnd.Rannyu(),4));
    ...
    void sort_paths() {
        std::sort(_itinera.begin(), _itinera.end(),[&](const path& a, const path& b){return a.length(_map) < b.length(_map);});
    }
